"use client";
import { X, Flame, Thermometer, Snowflake, Sparkles, Mail, Copy, Check, Pencil, Save, Clock, Phone, BookOpen, Layers, Loader2 } from "lucide-react";
...
type TabType = "overview" | "sequence" | "script";
  ...
  const getTimelineHistory = () => {
    ...
    if (lead.generated_email) {
      history.push({
        ...
        desc: "Gemini AI drafted a personalized sales cold outreach draft.",
        time: "Recent Update",
      });
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-backdrop">
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl border border-gray-100 dark:border-slate-800/80 animate-modal-content">
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-gray-100 dark:border-slate-800/60 px-6 shrink-0">
          {[
            ...
            return (
              <button
                ...
                className={clsx(
                  "py-3.5 px-4 text-xs font-semibold border-b-2 flex items-center gap-1.5 transition-colors -mb-px",
                  activeTab === tab.id
                    ? "border-blue-500 text-blue-600 dark:text-blue-400"
                    : "border-transparent text-gray-450 hover:text-gray-700 dark:hover:text-slate-350"
                )}
              >
        ...
        <div className="flex-1 flex flex-col overflow-hidden pr-1.5">
                ...
                <div className="relative ml-4 space-y-5">
                  ...
                  {getTimelineHistory().map((item, idx) => (
                    <div
                      key={idx}
                      className={clsx(
                        "relative pl-6 animate-list-item",
                        idx === 0 && "delay-1",
                        idx === 1 && "delay-2",
                        idx === 2 && "delay-3",
                        idx === 3 && "delay-4",
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-500 dark:text-slate-400 mt-1 leading-relaxed">{item.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === "sequence" && (
            <div className="space-y-6">
              ...
              <div className="relative border-l-2 border-blue-500 pl-4 space-y-3">
                ...
                {lead.generated_email ? (
                  <div className="bg-slate-900 dark:bg-slate-950/80 border border-slate-800/80 rounded-xl p-4 relative hover-lift hover-scale hover:border-slate-700/80 transition-all">
                    {isEditing ? (
              ...
              <div className="relative border-l-2 border-purple-400 pl-4 space-y-3">
                ...
                <div className="bg-purple-50 dark:bg-slate-900 border border-purple-100 dark:border-slate-700/60 rounded-xl p-4 hover-lift hover-scale hover:border-purple-200 dark:hover:border-slate-700 transition-all">
              ...
              <div className="relative border-l-2 border-gray-300 dark:border-slate-600 pl-4 space-y-3">
                ...
                <div className="bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700/60 rounded-xl p-4 hover-lift hover-scale hover:border-gray-350 dark:hover:border-slate-700 transition-all">
                  <pre className="text-xs text-gray-600 dark:text-slate-400 whitespace-pre-wrap font-mono leading-relaxed">
                    {day5Breakup}
                  </pre>
                </div>
              </div>
          ...
          {activeTab === "script" && (
            <div className="space-y-6">
                  ...
                  >
                    {scriptLoading ? (
                      ...
                      <>
                        ...
                        {callScript ? "Regenerate Script" : "Generate Script"}
                      </>
                    )}
                  </button>
                </div>
              ...
              {callScript ? (
                <div className="bg-slate-900 dark:bg-slate-950/80 border border-slate-800/80 rounded-xl p-4 relative space-y-4 hover-lift hover-scale hover:border-slate-700/80 transition-all">
                  <pre className="text-xs text-slate-300 whitespace-pre-wrap font-mono leading-relaxed">
                    {callScript}
                  </pre>
                  <button
